...
export class SpecialPackageService {
  ...
  async addSpecialPackage(
    ...
    try {
      ...
      const mData = { ...addSpecialPackageDto, ...defaultData };
    ...
    try {
      let data: any = await this.specialPackageModel
        ...
        .populate('products.product',
          'name description totalPages currentVersion translatorName publishedDate shortDescription author  salePrice sku tax shortDesc discountType slug variations hasVariations variationsOptions discountAmount images quantity category subCategory brand tags unit _id',
        )
        .select(select);
      ...
      const newdata: any = await data.products.map((item) => {
        let transFrom = { ...item.product._doc, ...{ quantity: item.quantity, hasVariations: item.hasVariations, selectedVariation: item.selectedVariation } }
        ...
        if (transFrom.hasVariations) {
          ...
          if (!found) {
            return { ...transFrom, ...{ hasVariations: false, selectedVariation: null } }
            // Code for Delete variation
          } else {
            return { ...transFrom, ...{ hasVariations: true, selectedVariation: found } };
          }
        } else {
          return transFrom
        }
